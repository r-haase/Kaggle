...
# building one multiclass model with multiple hidden layers
#neurons <- c(100, 95, 90, 85, 80, 75, 70)
neurons <- c(65, 60, 55, 50, 45, 40, 35, 30)
#PREPORCESS data
training_data <- scale(training_data[,2:94], center = TRUE, scale = TRUE)
training_data <- cbind(training_data, targets)
#for (i in 1:length(lambdas)) {
        for (j in 1:length(neurons)) {
#                print(i)
                print(j)
                
                nnetFit <- neuralnet(fmla, training_data, hidden = c(70,70,70), threshold = 1, linear.output = FALSE,
                                     lifesign = "full", lifesign.step = 100, act.fct = "logistic",
                                     err.fct = "sse", learningrate = 0.01)
                #startweights = weightsStart,
                #linear.output = TRUE,
                #, 
                ...
                pred_holdout <- compute(nnetFit, holdout_sample[,2:94])$net.result
                # calculate and save cost
                cost <- rbind(cost, c(neurons[j], sum((targets_cv-pred_holdout)^2)))
                
                #pred <- compute(nnetFit, test_data[,2:94])$net.result
                #predictions <- pred
}
#}
# add id column
...
write.csv(predictions, file = "./resultsMulticlassNeuralnetT1.csv", row.names = FALSE)
