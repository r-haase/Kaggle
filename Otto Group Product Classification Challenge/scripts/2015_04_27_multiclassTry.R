...
lambdas <- c(0.1, 0.3, 1)
neurons <- c(95,93, 90, 87, 85, 83, 80, 75)

# building one multiclass model
...
for (i in 1:length(lambdas)) {
        for (j in 1:length(neurons)) {
                        ...
                        nnetFit <- pcaNNet(training_data[,2:94], targets, maxit = 500, thresh = .99, size = neurons[j], decay = lambdas[i], MaxNWts = 10000, pc = c("center", "scale"))
...
write.csv(predictions, file = "./resultsMulticlassPCA99.csv", row.names = FALSE)
