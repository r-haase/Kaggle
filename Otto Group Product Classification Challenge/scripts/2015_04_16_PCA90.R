...
lambdas <- c(0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 1, 3)
neurons <- c(90, 80, 75, 70, 68, 65, 63, 60, 58, 55, 53, 50, 40, 30)
...
for (k in 1:9) {
        for (i in 1:length(lambdas)) {
                for (j in 1:length(neurons)) {
                        ...
                        nnetFit <- pcaNNet(training_data[,2:94], targets[,k], maxit = 500, thresh = .90, size = neurons[j], decay = lambdas[i], MaxNWts = 10000, pc = c("center", "scale"))
...
#write.csv(predictions, file = "./pca90.csv", row.names = FALSE)
