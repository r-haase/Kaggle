...
# transfrom training_data into a matrix

for (i in 1:9) {
        
        nnetFit <- brnn(as.matrix(training_data[,2:94]), targets[,i], epochs = 4, neurons = 47, verbose = TRUE, normalize = TRUE, cores = 1)
...
write.csv(predictions, file = "./results6.csv", row.names = FALSE)
