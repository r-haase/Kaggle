...
kays <- c(1,3,5,7,9,11,13,15)


for (i in 1:length(kays)) {
        print(i)
        
        
        #myKKNN <- kknn(train = training_data[,2:94], test = holdout_sample[,2:94] , learn = targets, valid = targets_cv, 
         #              k = kays[i], kernel = "optimal", scale = TRUE)
        myKKNN <- kknn(fmla, training_data , test_data, k = 5, kernel = "optimal", scale = TRUE, distance = 2) 
        # save model
        # models <- rbind(models, c(k, lambdas[i], neurons[j], nnetFit))
        # make predictions on holdout sample
        #pred_holdout <- predict(myKKNN, holdout_sample[,2:94], probability = TRUE)
        
        #matches <- ifelse(holdout_sample[,95] == pred_holdout, 0, 1) 
        ...
        #cost <- rbind(cost, c(i, sum(matches)/48378))
...
write.csv(predictions, file = "./Kknn_5_LOGGED.csv", row.names = FALSE)
