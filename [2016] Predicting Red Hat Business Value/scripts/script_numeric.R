...
trainingData_ONE[,cols] <- lapply(trainingData_ONE[,cols], as.numeric)
#trainingData_ONE$outcome <- as.logical(trainingData_ONE$outcome)
#trainingData_ONE$outcome <- as.numeric(trainingData_ONE$outcome)
trainingData_ONE$char_38 <- (trainingData_ONE$char_38 - mean(trainingData_ONE$char_38))/sd(trainingData_ONE$char_38)
#trainingData_ONE[20:48] <- preProcess(trainingData_ONE[20:48], method = c("center", "scale")) 
...
trainingData_REST[,cols] <- lapply(trainingData_REST[,cols], as.numeric)
#trainingData_REST$outcome <- as.logical(trainingData_REST$outcome)
#trainingData_REST$outcome <- as.numeric(trainingData_REST$outcome)
...
write.csv(results, file = "submission_7th_numerics_ROC_attempt_char10_13levels.csv", row.names = FALSE)
